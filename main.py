...
def pretrain_vqvae(model, x):
    print("Pretraining VQVAE...")
    optimizer = Adam(model.parameters(), lr=opt.args.lr)
    for epoch in tqdm.tqdm(range(opt.args.rec_epoch)):
        z_qua,comloss = model.encoder(x)
        x_hat = model.decoder(z_qua)
...
def train(model, X1, A1, X2, A2, y):
    ...
    for epoch in pbar:
        ...
        if opt.args.first_view == 'RNA':
            ...
            elif abs(losslist[epoch-1]-losslist[epoch-2])<abs(losslist[epoch-2]-losslist[epoch-3]):
                L_KL1 = distribution_loss(Q1, target_distribution(Q1[0].data))
                L_KL2 = distribution_loss(Q2, target_distribution(Q1[0].data))
            else:
        ...
        lossvalue = loss.detach().cpu().item()
    ...
    integrated_z = ((Z1 + Z2) / 2).cpu().detach().numpy()
    np.save(f'./input/Ma/integrated_z.npy', integrated_z)
...
if __name__ == '__main__':
    ...
    vqvae1 = VQVAE(
        vqvae_n_enc_1=opt.args.ae_n_enc_1, vqvae_n_enc_2=opt.args.ae_n_enc_2,
        vqvae_n_dec_1=opt.args.ae_n_dec_1, vqvae_n_dec_2=opt.args.ae_n_dec_2,
        n_input=opt.args.n_d1, n_z=opt.args.n_z,
        num_embeddings=256,embedding_dim=opt.args.n_z).to(opt.args.device)
    ...
    if opt.args.pretrain:
        t0 = time()
        pretrain_vqvae(vqvae1, Xr)
        pretrain_vqvae(vqvae2, Xa)

        pretrain_gae(gae1, Xr, Ar)
        pretrain_gae(gae2, Xa, Aa)

        model = scHEAGC(vqvae1, vqvae2, gae1, gae2, n_node=Xr.shape[0]).to(opt.args.device)
        ...
        print(f"Current memory usage: {current / 10**9} GiB")
        print(f"Peak memory usage: {peak / 10**9} GiB")
