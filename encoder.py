...
class VQVAE_encoder(nn.Module):
    def __init__(self, vqvae_n_enc_1, vqvae_n_enc_2, n_input, n_z, num_embeddings, embedding_dim):
        super(VQVAE_encoder, self).__init__()
        self.enc_1 = Linear(n_input, vqvae_n_enc_1)
        self.enc_2 = Linear(vqvae_n_enc_1, vqvae_n_enc_2)
        self.z_layer = Linear(vqvae_n_enc_2, n_z)
    ...
    def forward(self, x):
        ...
        z_vqvae = self.z_layer(z)
        z_quantized, indices, commitment_loss = self.vector_quantization(z_vqvae)
        return z_quantized, commitment_loss

    def vector_quantization(self, z_vqvae):
        z_flat = z_vqvae.view(-1, z_vqvae.size(-1))
        distances = torch.sum(z_flat**2, dim=1, keepdim=True) + torch.sum(self.codebook.weight**2, dim=1) - 2 * torch.matmul(z_flat, self.codebook.weight.t())
        ...
        z_quantized = z_quantized.view(z_vqvae.size())
        return z_quantized, indices, commitment_loss


class VQVAE_decoder(nn.Module):
    def __init__(self, vqvae_n_dec_1, vqvae_n_dec_2, n_input, n_z):
        super(VQVAE_decoder, self).__init__()

        self.dec_1 = Linear(n_z, vqvae_n_dec_1)
        self.dec_2 = Linear(vqvae_n_dec_1, vqvae_n_dec_2)
        self.x_bar_layer = Linear(vqvae_n_dec_2, n_input)
        self.act = nn.LeakyReLU(0.2, inplace=True)

    def forward(self, z_qua):
        z = self.act(self.dec_1(z_qua))
...
class VQVAE(nn.Module):
    def __init__(self, vqvae_n_enc_1, vqvae_n_enc_2, vqvae_n_dec_1, vqvae_n_dec_2, n_input, n_z,
                 num_embeddings, embedding_dim):
        super(VQVAE, self).__init__()

        self.encoder = VQVAE_encoder(
            vqvae_n_enc_1=vqvae_n_enc_1,
            vqvae_n_enc_2=vqvae_n_enc_2,
            n_input=n_input,
            n_z=n_z,
        ...
        self.decoder = VQVAE_decoder(
            vqvae_n_dec_1=vqvae_n_dec_1,
            vqvae_n_dec_2=vqvae_n_dec_2,
            n_input=n_input,
            n_z=n_z)
